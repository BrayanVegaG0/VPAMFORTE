...
class DinardapSoapRemoteDataSourceHttp {
  ...
  Future<Map<String, String?>> consultarPorCedula(String cedula) async {
    ...
    if (creds == null) {
      throw Exception('No hay credenciales BasicAuth guardadas. Inicie sesión nuevamente.');
    }

    final basic = 'Basic ${base64Encode(utf8.encode('${creds.username}:${creds.password}'))}';
    ...
    final resp = await client.post(
      ...
      headers: {
        ...
        'Authorization': basic,
        'SOAPAction': 'https://siimiesalpha.desarrollohumano.inclusion.gob.ec/encuestaMovil/ConsultaInfRegCivil',
      },
      body: envelope,
    );
    ...
    dev.log('DINARDAP RESPONSE status=${resp.statusCode} reason=${resp.reasonPhrase}', name: 'DINARDAP');
    ...
    if (upper.contains('CEDULA INVALIDA') || upper.contains('CÉDULA INVALIDA')) {
...
// SOAP Fault aunque haya 200 (a veces)
  ...
  String _extractSoapFaultOrBody(String body) {
    ...
    final faultString = RegExp(r'<faultstring>([\s\S]*?)</faultstring>', caseSensitive: false)
        .firstMatch(body)
        ?.group(1)
        ?.trim();

    final detail = RegExp(r'<detail>([\s\S]*?)</detail>', caseSensitive: false)
        .firstMatch(body)
        ?.group(1)
        ?.trim();
